"""
generate_fastdeliver.py
--------------------
Generates a synthetic FastDeliver A/B test dataset with intentional
data quality issues for analytical exercises.

Output: fastdeliver_raw.csv (same directory as this script)
"""
...
OUTPUT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "fastdeliver_raw.csv")
...
def build_delivery_timestamps(
    ...
    if status == "cancelled":
        ...
        return {
            ...
            "stage_3_courier_assigned_at": None,
            "stage_4_courier_arrived_restaurant_at": None,
            "stage_5_order_picked_up_at": None,
            "stage_6_courier_near_customer_at": None,
    ...
    return {
        ...
        "stage_3_courier_assigned_at": stage_times[2],
        "stage_4_courier_arrived_restaurant_at": stage_times[3],
        "stage_5_order_picked_up_at": stage_times[4],
        "stage_6_courier_near_customer_at": stage_times[5],
        "stage_7_delivered_at": stage_times[6],
        "courier_assigned_at": stage_times[2],
...
def main() -> None:
    print("Generating FastDeliver synthetic dataset...")
    ...
    courier_ids_pool = [f"DASH_{str(i).zfill(4)}" for i in range(1, N_DASHERS + 1)]
    ...
    for i in range(N_ORDERS):
        ...
        # Courier assignment (only for delivered/in_progress)
        if status in ("delivered", "in_progress"):
            courier_id = RNG.choice(courier_ids_pool)
            delivery_id = f"DEL_{str(i).zfill(6)}"
        else:
            courier_id = RNG.choice(courier_ids_pool)  # even cancelled may have been assigned briefly
            delivery_id = f"DEL_{str(i).zfill(6)}"
        ...
        row = {
            ...
            "courier_assigned_at": stages["courier_assigned_at"].strftime("%Y-%m-%dT%H:%M:%S") if stages["courier_assigned_at"] else None,
            ...
            "stage_3_courier_assigned_at": stages["stage_3_courier_assigned_at"].strftime("%Y-%m-%dT%H:%M:%S") if stages["stage_3_courier_assigned_at"] else None,
            "stage_4_courier_arrived_restaurant_at": stages["stage_4_courier_arrived_restaurant_at"].strftime("%Y-%m-%dT%H:%M:%S") if stages["stage_4_courier_arrived_restaurant_at"] else None,
            ...
            "stage_6_courier_near_customer_at": stages["stage_6_courier_near_customer_at"].strftime("%Y-%m-%dT%H:%M:%S") if stages["stage_6_courier_near_customer_at"] else None,
            ...
            "has_missing_courier_flag": False,
    ...
    # 1. Missing courier (1%) — for delivered orders only to be realistic
    delivered_mask = df["status"] == "delivered"
    delivered_idx = df[delivered_mask].index.tolist()
    n_missing_courier = int(n_total * 0.01)
    missing_courier_idx = RNG.choice(delivered_idx, size=n_missing_courier, replace=False)
    df.loc[missing_courier_idx, "courier_id"] = None
    df.loc[missing_courier_idx, "courier_assigned_at"] = None
    df.loc[missing_courier_idx, "stage_3_courier_assigned_at"] = None
    df.loc[missing_courier_idx, "has_missing_courier_flag"] = True
    ...
    for idx in ts_issue_idx:
        ...
        s4 = df.at[idx, "stage_4_courier_arrived_restaurant_at"]
        s5 = df.at[idx, "stage_5_order_picked_up_at"]
        df.at[idx, "stage_4_courier_arrived_restaurant_at"] = s5
    ...
    FINAL_COLUMNS = [
        ...
        "delivery_id", "courier_id", "courier_assigned_at", "pickup_at",
        "delivered_at", "delivery_duration_minutes",
        "stage_1_order_placed_at", "stage_2_restaurant_confirmed_at",
        "stage_3_courier_assigned_at", "stage_4_courier_arrived_restaurant_at",
        "stage_5_order_picked_up_at", "stage_6_courier_near_customer_at",
        "stage_7_delivered_at",
        "has_duplicate_flag", "has_timestamp_issue_flag",
        "has_missing_courier_flag", "has_outlier_flag",
    ]
    df = df[FINAL_COLUMNS]
