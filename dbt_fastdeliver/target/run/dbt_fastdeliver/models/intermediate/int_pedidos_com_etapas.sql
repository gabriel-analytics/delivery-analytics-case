  ...
  create view "fastdeliver"."main_intermediate"."int_pedidos_com_etapas__dbt_tmp" as (
    with stg as (
    select * from "fastdeliver"."main_staging"."stg_pedidos"
    -- Apenas pedidos entregues, sem flags de dados problemáticos
    where is_delivered = true
      and has_missing_courier_flag = false
...
etapas as (
    select
        ...
        round(
            (epoch(stage_4_courier_arrived_restaurant_at) - epoch(stage_2_restaurant_confirmed_at)) / 60.0,
            2
        ) as duracao_preparo_min,

        -- Etapa 3: Atribuição do courier (do pedido até courier ser designado)
        round(
            (epoch(stage_3_courier_assigned_at) - epoch(stage_1_order_placed_at)) / 60.0,
            2
        ) as duracao_atribuicao_min,

        -- Etapa 4: Coleta (courier pega o pedido no restaurante)
        round(
            (epoch(stage_5_order_picked_up_at) - epoch(stage_4_courier_arrived_restaurant_at)) / 60.0,
            2
        ...
        round(
            (epoch(stage_7_delivered_at) - epoch(stage_6_courier_near_customer_at)) / 60.0,
            2
